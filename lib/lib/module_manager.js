...
ModuleManager.prototype = {
    ...
    */
    initialise: function(modules){
        modules = path.normalize(this.join([__dirname, '..', modules]));
        var middleware = 'middleware';
        var routes = 'routes';
        list = fs.readdirSync(modules);
        for (var i = 0; i < list.length; i++) {
            var module = this.join([modules, list[i]]);
    ...
    */
    scanModule: function(module, part) {
        ...
        if (fs.statSync(module).isDirectory()) {
            var list = fs.readdirSync(this.join([module, part]));
            for (var i = 0; i < list.length; i++) {
                var path = this.join([module, part, list[i]]);
